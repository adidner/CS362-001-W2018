...
public class TimeTable {
     ...
     **/
	  public LinkedList<CalDay> getApptRange(LinkedList<Appt> appts,GregorianCalendar firstDay, GregorianCalendar lastDay)throws DateOutOfRangeException{
		...
		//this was the spot of a bug but I had to change it back to get coverage and not infinate loops while testing			
		for (int i = 1; i < appts.size(); i++) {
			...
			while (itr.hasNext()) {
				...
				//System.out.println("In OUter whileO");
	   ...
	   /**
	    ...
	    private static LinkedList<GregorianCalendar> getApptOccurences(Appt appt, 
	        ...
	        //Make sure that the firstDay is before the last day DOWN DWON ODWN
	        if (!firstDay.before(lastDay)) {//REDUDANT CALL IMPOSSIBLE TO COVER
	            return result;
	        }
